...
if ($action == 'userregister') {
    if (GETPOST('allowregister', 'alpha') != 1) {
        $allowregister = 0;
    } else {
        $allowregister = GETPOST('allowregister', 'alpha');
    }
}
...
if (($action == 'userregister') && !$error){
    $res = dolibarr_set_const($db,'GC_ALLOW_INSERT_USERS',$allowregister);
    if (!$res > 0) { $error++; }
    if (!$error) {
        $db->commit();
        $mesg = '<div class="ok">' . $langs->trans("Saved") . '</div>';
...
echo '<br>',
    '<form method="POST" action="', $_SERVER['PHP_SELF'], '">',
    '<fieldset>',
    '<legend>', $langs->trans('AllowTheRegister'), '</legend>',
    '<input type="hidden" name="token" value="', $_SESSION['newtoken'], '">',
    '<input type="hidden" name="action" value="userregister">',
    '<label for="allowregister">', $langs->trans('AllowRegister'),'</label> ',
    '<input id="chk_allow_register" type="checkbox" name="allowregister" value="',$conf->global->GC_ALLOW_INSERT_USERS,'" checked="">',
    '<input type="submit" class="gc_btn" value ="', $langs->trans("Save"), '">',
    '</fieldset>',
    '</form>',
    '<br>';
