...
if ($conf->global->GC_ORIGIN_CALL != "/"){
	$original_php_self = "/".$conf->global->GC_ORIGIN_CALL."/index.php?mainmenu=home";
} else {
	$original_php_self = "/index.php?mainmenu=home";
}
...
if ($actual_link == $server_root || $actual_link == $server_index || $actual_link == $server_logout) {

	$control = new Control($conf, $db);
	$Opauth = $control->buildOpauth();

	if ($Opauth == false) {
		echo '<div style="text-align: center; margin: 0px auto; max-width: 560px;"><p><strong style="color: red;">Error: </strong>The request has been rejected. Please try again.</p></div><br>';
	}

} else if ($actual_link == $server_google) {
...
if ($php_self != $original_php_self) {
	$_SESSION['gc_php_self'] = $php_self;
}

if (!empty($_SESSION['gc_php_self'])) {
	$php_self = $_SESSION['gc_php_self'];
} else {
	$php_self = $original_php_self;
}
...
if (empty($response['auth']) || empty($response['timestamp']) || empty($response['signature']) || empty($response['auth']['provider']) || empty($response['auth']['uid'])) {
	...
	echo '<div style="text-align: center; margin: 0px auto; max-width: 560px;"><p><strong style="color: green;">OK: </strong>Credentials have been received from Google. Processing...'."</p></div><br>\n";
	echo '<script type="text/javascript">
		...
		if (false !== $email_bool){
			...
			if (!is_bool($key)){ // Finded, the email allready exists
				$obj->session_init($email);
        $_SESSION['mainmenu'] = $php_self;
			?>
				<script type="text/javascript"> window.location.href = <?php echo '"'.$php_self.'"'; ?>;  </script>
				<?php

			} else if($key == false && $conf->global->GC_ALLOW_INSERT_USERS == 1) { // Not finded? Saving is allowed?
				$theres = $obj->insertUser($response);
				if ($theres) {
					$obj->session_init($email);
          $_SESSION['mainmenu'] = $php_self;
				?>
					...
					echo '<div style="text-align: center; margin: 0px auto; max-width: 560px;"><p><strong style="color: red;">Error: </strong>'.$langs->trans('InsertError').'</p></div><br>\n';				
				}
				
			} else if($key == false && $conf->global->GC_ALLOW_INSERT_USERS == 0){ // Not finded, you cannot save
				echo '<div style="text-align: center; margin: 0px auto; max-width: 560px;"><p><strong style="color: red;">Error: </strong>'.$langs->trans('InsertsNotAllowed').'</p></div><br>\n';
			}

		} else { // The email does not match with the registered domains
			echo '<div style="text-align: center; margin: 0px auto; max-width: 560px;"><p><strong style="color: red;">Error: </strong>'.$langs->trans('AccessDenied').': <strong>'.$conf->global->GC_EMAIL_DOMAIN.'</strong></p></div><br>\n';
		}
	}
}
?>
