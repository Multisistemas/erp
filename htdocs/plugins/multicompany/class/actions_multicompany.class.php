						...
						if ($action != 'create' && $action != 'edit')
						{
							if ($object->isObjectUsed($object->id) === 0 && ((! empty($user->admin) && ! $user->entity) || ! empty($user->rights->multicompany->thirdparty->write)))
							{
								...
								if (! empty($selectEntities)) {
									...
									$this->resprints.= '<div class="refidno modify-entity-disabled" data-tooltip="'.$langs->trans("NoOtherEntityAvailable").'" data-tooltip-position="bottom">';
									$this->resprints.= '<span class="fa fa-globe"></span><span class="multiselect-selected-title-text">'.$this->label.'</span>';
									$this->resprints.= '</div>';
								}
							}
	...
	private function getTopRightMenu()
	{
		...
		if (! empty($conf->global->MULTICOMPANY_TRANSVERSE_MODE) || ! empty($user->admin))
		{
			...
			else
			{
				...
				$text ='<span class="fa fa-globe atoplogin switchentity'.($conf->global->MAIN_THEME === 'eldy' ? '' : ' padding-top3').'">';
				if ($conf->global->MAIN_THEME === 'eldy' && empty($conf->global->MULTICOMPANY_NO_TOP_MENU_ENTITY_LABEL)) $text.= '<span class="topmenu-mc-label">'.$this->label.'</span>';
				$text.= '</span>';
